...
namespace JobMonitor.Console
{
    class Program
    {
        const string _commandNamespace = "JobMonitor.Console.Commands";
        private static Dictionary<string, Dictionary<string, IEnumerable<ParameterInfo>>> _commandLibraries;

        static void Main(string[] args)
        {
            //System.Console.WriteLine("Please wait, prepare data");
            System.Console.Title = typeof(Program).Name;
            InitializeCommands();
 

            // Write out the result:
            WriteToConsole(Execute(new ConsoleCommand("Show")));
            Run( );
        }
        static void Run( )
        {
        ...
        static string Execute(ConsoleCommand command)
        {
            ...
            if (!methodDictionary.Any(x => x.Key.ToLower() == command.Name.ToLower()))
            {
                return badCommandMessage;
            }
            ...
            IEnumerable<ParameterInfo> paramInfoList = methodDictionary.First(x => x.Key.ToLower() == command.Name.ToLower()).Value.ToList();
            command.Name = methodDictionary.First(x => x.Key.ToLower() == command.Name.ToLower()).Key;
...
#pragma warning restore CS0168 // Variable is declared but never used
        ...
        private static void InitializeCommands()
        {
            ...
            var q = from t in Assembly.GetExecutingAssembly().GetTypes()
                    ...
                    select t;
            //var d = Assembly.GetExecutingAssembly().GetTypes().Where(t => t.IsClass
            //&& t.Namespace == _commandNamespace);
